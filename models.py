...
class WGAN(nn.Module):
    ...
    def _make_extra(self, layer_type, num_filters, n_extra_layers):
        modules = OrderedDict()
        ...
        for i in range(n_extra_layers):
            ...
            module = nn.Sequential(layer_type(num_filters, num_filters, kernel_size=3, stride=1, padding=1, bias=False),
...
class WGANDiscriminator(WGAN):
    def __init__(self, input_size=32, in_channels=3, num_filters=64, n_extra_layers=0, activation=nn.LeakyReLU):
        ...
        self.first_conv = nn.Sequential(nn.Conv2d(in_channels, num_filters, kernel_size=4, stride=2, padding=1),
                                        activation(inplace=True))
        if self.has_extra:
            self.extra = self._make_extra(nn.Conv2d, num_filters, n_extra_layers)
    ...
    def _make_conv(self, num_filters):
        ...
        for i in range(int(np.log2(self.input_size)) - 3):
            name = stage_name + "_{}".format(i + 1)
            module = nn.Sequential(nn.Conv2d(num_filters, num_filters*2, kernel_size=4, stride=2, padding=1, bias=False),
                                   nn.BatchNorm2d(num_filters*2),
                                   self.activation(inplace=True))
            num_filters *= 2
            modules[name] = module
